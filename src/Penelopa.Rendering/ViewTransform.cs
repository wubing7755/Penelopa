...
/// view coordinates (the render target's pixels, Y grows down). The view is
/// rendered at physical resolution: one world unit equals one CSS pixel, so
/// the render target is <c>DevicePixelRatio</c> times the CSS size. Rendering
/// and hit testing share the same instance so what the user sees is always
/// what the pointer hits, at any display DPI.
/// </summary>
/// <remarks>
...
public readonly struct ViewTransform
{
    ...
    public ViewTransform(int viewWidth, int viewHeight, float devicePixelRatio)
    {
    ...
    public SKPoint WorldToView(float worldX, float worldY)
        => new(worldX * DevicePixelRatio, ViewHeight - worldY * DevicePixelRatio);

    /// <summary>Maps a view (render target) pixel back to world space.</summary>
    public SKPoint ViewToWorld(float viewX, float viewY)
        => new(viewX / DevicePixelRatio, (ViewHeight - viewY) / DevicePixelRatio);
    ...
    /// <summary>
    /// Puts the canvas into world-coordinate drawing mode (origin at the
    /// bottom-left, Y grows up, scaled by the device pixel ratio). Drawing in
    /// world coordinates then lands on the correct physical pixels.
    /// </summary>
